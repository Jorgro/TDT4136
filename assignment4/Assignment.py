...
class CSP:
    ...
    def backtrack(self, assignment):
        ...
        """
        # Check if we have a solution, return assignment if so
        solution = True
        ...
        for val in assignment[var]:  # no ordering done..
            # check if value is consistent with assignment:
            assignment_copy = copy.deepcopy(assignment)
            consistent = True
            if len(self.get_all_neighboring_arcs(var)) == 0:
                consistent = True
            for arc in self.get_all_neighboring_arcs(var):
                count = 0
                for j in assignment[arc[0]]:
                    if (j, val) in self.constraints[arc[0]][var]:
                        count += 1
                if count == 0:
                    consistent = False
    ...
    def select_unassigned_variable(self, assignment):
        ...
        """
        # MVR solution
        smallest = 20000  # just a very large number
        for i in assignment:
            length = len(assignment[i])
            if length == 0:  # failure detected immediately and search done
                return i
            if length < smallest and length > 1:
                smallest = length
                chosen = i
        return chosen
